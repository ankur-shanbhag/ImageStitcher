...
public class DBScanVersion1Mapper extends Mapper<LongWritable, Text, Text, NullWritable> {
    ...
    protected void setup(Mapper<LongWritable, Text, Text, NullWritable>.Context context)
        ...
        try {
            ...
            threshold = Double.parseDouble(conf.get(HdfsConstants.IMAGE_MATCHING_ERROR, "0.0"));
    ...
    // private boolean checkIfSimilar(Collection<DataObject> sourceCluster,
    // Collection<DataObject> targetCluster) {
    //
    // double[] sourceCenterPoint =
    // DataAnalyser.computeArithmeticMean(sourceCluster);
    // double[] targetCenterPoint =
    // DataAnalyser.computeArithmeticMean(targetCluster);
    //
    ...
    // if (Math.abs(distanceFromSourceCenter[i] - distanceFromTargetCenter[i]) >
    // threshold)
    // return false;
    // }
    //
    ...
    private boolean checkIfSimilar(List<DataObject> sourceCluster,
        ...
        Map<DataObject, List<DataObjectRelation>> sourceRelationMap = computeRelations(sourceCluster);
        Map<DataObject, List<DataObjectRelation>> targetRelationMap = computeRelations(targetCluster);
    ...
    private double computeErr(DataObjectRelation obj1, DataObjectRelation obj2) {
    ...
    private Map<DataObject, List<DataObjectRelation>> computeRelations(List<DataObject> cluster) {

        Map<DataObject, List<DataObjectRelation>> allNeighborsRelationMap = new HashMap<>();
